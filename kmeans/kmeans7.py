...
def get_vgg_arr(data_list):
    image_info = []
    image_dir = 'eng-wiki/image_vgg19_pool5/'
...
def preprocess():
    train_file_path = 'eng-wiki/image_vgg19_pool5_train.txt'
    val_file_path = 'eng-wiki/image_vgg19_pool5_val.txt'
    ...
    return all_image_arr,train_image_num, train_image_arr ,train_list,val_image_num, val_image_arr,val_list

def clustering(all_image_arr):
    iteration = 200
...
def build_graph(k,neigh_idx,neigh_num):
    ...
    graph_path='eng-wiki/image_graph_k%d.npy' % k
...
def build_feature_file(image_arr,image_num,data_list):
    ...
    feature_dir='eng-wiki/image_feature_k%d' % k
...
k=sys.argv[1]
call_graph=sys.argv[2]
call_train=sys.argv[3]
call_test=sys.argv[4]

all_image_arr, train_image_num, train_image_arr, train_list,val_image_num, val_image_arr,val_list=preprocess()

neigh_num = 8
# neigh_idx,cluster_cent=clustering(all_image_arr)
neigh_idx=0
cluster_cent=0
